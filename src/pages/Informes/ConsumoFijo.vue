      ...
      <q-form ref="formordenEntrada" >
        <div class="q-pa-md">
          <div class="row q-col-gutter-sm">
            <div class="col-xs-12 col-sm-12 col-md-12">
              <div class="row q-col-gutter-sm">
                <q-input class="col-xs-12 col-sm-6 col-md-3 q-pt-sm" 
                  ...
                  v-model="ConsumoFijo.Codigo" outlined  
                  >
                ...
                <q-input class="col-xs-12 col-sm-6 col-md-3 q-pt-sm" 
                  ...
                  v-model="ConsumoFijo.Ruta" outlined  
                  >
                  <template v-slot:prepend>
                ...
                <q-input class="col-xs-12 col-sm-6 col-md-6 q-pt-sm" 
                  ...
                  v-model="ConsumoFijo.NombreCompleto" outlined  
                  >
                  <template v-slot:prepend>
            ...
            <div class="q-pa-md q-gutter-md">
              <q-btn label="Consultar" icon="search" outline align="center" @click="consultar(ConsumoFijo)" unelevated >
              </q-btn>
            </div>
            <div class="q-pa-md q-gutter-md">
        ...
        <div class="q-pa-md q-gutter-md">
            ...
            <template v-slot:body-cell-Seleccionar="props">
                ...
                <q-btn :icon="props.row.Icono" :color="props.row.IconoColor" align="center" outline label="" @click="seleccionarConsumoFijo(props.row)"/>
                </q-td>
            </template>
            <template v-slot:body-cell-ConsumoFijo="props">
              <q-td key="ConsumoFijo" :props="props" auto-width>
                {{ props.row.ConsumoFijo }}
                <q-popup-edit label-set="Ingresar" label-cancel="Cancelar" v-model="props.row.ConsumoFijo" title="Ingrese Consumo Fijo" buttons>
                    <q-input type="number" v-model="props.row.ConsumoFijo" autofocus>
                    <template v-slot:prepend>
...
export default {
  ...
  data () {
    return {
      model: null,
      ListaSuscriptores: [],
      ConsumoFijo: { Seleccionar: 0, IdSuscriptor: 0, Codigo: '', Ruta: '', NombreCompleto: '', Direccion: '', ConsumoFijo: 0, IsSuspendido: -1, IdEstadoSuscriptor: -1, IconoColor: 'blue', Icono: 'edit' },
      filterSuscriptores: '',
      todasCriticas: false,
      lisConsumosFijosModificar: [],
      options: [],
      ...
      fecha: date,
      usuario: {},
      vcSuscriptor: ['Seleccionar', 'Codigo', 'Ruta', 'NombreCompleto', 'ConsumoFijo'],
      columnsSuscriptor: [
  ...
  methods: {
    exportTable (tablaE, columnasE) {
      ...
      var nombreArchivo = 'ConsumoFijo_' + this.ConsumoFijo.Codigo
    ...
    Actualizar () {
      if (this.lisConsumosFijosModificar.length > 0) {
        this.modificarConsumosFijos(this.lisConsumosFijosModificar)
      } else {
    ...
    limpiar () {
      this.ConsumoFijo = { Seleccionar: 0, IdSuscriptor: 0, Codigo: '', Ruta: '', NombreCompleto: '', Direccion: '', ConsumoFijo: 0, IsSuspendido: -1, IdEstadoSuscriptor: -1, IconoColor: 'blue', Icono: 'edit' }
      this.lisConsumosFijosModificar = []
    ...
    consultar (CF) {
      ...
      api.post('/serviWeb/ConsumoFijo/', CF)
        .then((response) => {
          const consumosFijos = response.data
          consumosFijos.forEach(function (usuario) {
    ...
    getSelectedString () {
      return this.lisConsumosFijosModificar.length === 0 ? '' : `${this.lisConsumosFijosModificar.length} seleccionado${this.lisConsumosFijosModificar.length > 1 ? 's' : ''} de ${this.lisIncidencias.length} seleccionados`
    },
    getSelectdMedidores () {
      return this.lisMedidores.length === 0 ? '' : `${this.lisMedidores.length} medidor${this.lisMedidores.length > 1 ? 'es' : ''} `
    },
    seleccionarConsumoFijo (fila) {
      const self = this
      var i = this.lisConsumosFijosModificar.indexOf(fila)
      if (fila.Icono === 'edit') {
        fila.Icono = 'save'
        fila.IconoColor = 'red'
        self.ConsumoFijo.Codigo = fila.Codigo
        self.ConsumoFijo.NombreCompleto = fila.NombreCompleto
        self.ConsumoFijo.Ruta = fila.Ruta
        self.lisConsumosFijosModificar.push(fila)
      } else {
        fila.Icono = 'edit'
        fila.IconoColor = 'blue'
        self.lisConsumosFijosModificar.splice(i)
        self.ConsumoFijo.Codigo = ''
        self.ConsumoFijo.NombreCompleto = ''
        self.ConsumoFijo.Ruta = ''
      }
    },
    regresar () {
      this.$router.push('/admin')
    },
    modificarConsumosFijos (listaConsumosFijos) {
      const self = this
      self.$q.loading.show()
      api.post('/serviWeb/modificarConsumosFijos/', listaConsumosFijos)
        .then((response) => {
          self.lisConsumosFijosModificar = []
          self.consultar(self.ConsumoFijo)
