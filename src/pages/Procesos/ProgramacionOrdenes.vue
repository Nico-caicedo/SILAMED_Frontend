  ...
  <q-layout container style="height: 90vh">
      ...
      <q-toolbar>
        ...
        <q-avatar>
          <q-btn
            ...
            dense
            icon="print"
            @click="isDialogComponenteConsulta = true"
          />
        </q-avatar>
        <q-avatar>
          <q-btn
        ...
        <q-toolbar-title> Programación Ordenes de Trabajo </q-toolbar-title>
      </q-toolbar>
    </q-header>
    <q-page-container>
      <q-form ref="formordenEntrada" @submit="guardarProgramacion">
        <div class="q-pa-md">
          <div class="row q-col-gutter-sm">
            <q-select
              class="col-xs-12 col-md-5 q-field--with-bottom"
              outlined
                ...
                <q-item v-bind="scope.itemProps" v-on="scope.itemEvents">
                  <q-item-section>
                    ...
                    <q-item-label caption
                      >Descripción Banco:
                      {{ scope.opt.Descripcion_banco }}</q-item-label
                    >
                    <q-item-label caption
                      >Tipo Banco: {{ scope.opt.Tipo_banco }}</q-item-label
                    >
                    <q-item-label caption
                      >Líneas Banco: {{ scope.opt.Lineas_banco }}</q-item-label
                    >
                    <q-item-label caption
                      >Capacidad Medidores:
                      {{ scope.opt.Medidores_banco }}</q-item-label
                    >
                  </q-item-section>
                </q-item>
              </template>
            ...
            <q-select
              class="col-xs-12 col-md-4 q-field--with-bottom"
              outlined
              v-model="condicionMontada.Id_rangomedicion"
              map-options
              emit-value
              option-value="Id_rangomedicion"
              ...
              <template v-slot:option="scope">
                <q-item v-bind="scope.itemProps" v-on="scope.itemEvents">
                  <q-item-section aria-checked="Id_rangomedicion">
                    ...
                    <q-item-label caption
                      >IdRango: {{ scope.opt.Id_rangomedicion }}</q-item-label
                    >
                  </q-item-section>
                </q-item>
              </template>
            ...
            <q-select
              class="col-xs-12 col-md-3 q-field--with-bottom"
              outlined
              v-model="condicionMontada.Diametro_medidor"
              map-options
              emit-value
              option-value="Diametro_medidor"
                ...
                <template v-slot:body-cell-Modificar_caudal="props">
                  <q-td key="Id_caudal" :props="props" auto-width>
                    <q-btn
                      ...
                      align="center"
                      outline
                      @click="cargarInstrumentoscaudal(props.row)"
                      v-if="props.row"
                    >
                    </q-btn>
                  </q-td>
            ...
            <div class="col-xs-12 col-sm-12 col-md-8">
              <div class="row q-col-gutter-sm">
                <q-select
                  class="col-xs-12 col-md-6 q-field--with-bottom"
                  outlined
                  v-model="ordenEntradaD.Id_normaref"
                  map-options
                  emit-value
                  option-value="Id_normaref"
                    ...
                    <q-item v-bind="scope.itemProps" v-on="scope.itemEvents">
                      <q-item-section aria-checked="Id_normaref">
                        ...
                        <q-item-label caption
                          >Parámetros:
                          {{
                            scope.opt.LisNormaInfoTecnica.length
                          }}</q-item-label
                        >
                      </q-item-section>
                    </q-item>
                ...
                <q-input
                  class="col-xs-6 col-sm-6 col-md-4 q-field--with-bottom"
                  outlined
                  stack-label
                  ...
                  <template v-slot:append>
                    <q-icon name="access_time" class="cursor-pointer">
                      ...
                      >
                        <q-time
                          v-model="
                            ProgramacionOrden.Horacalibracion_programacionorden
                          "
                        >
                ...
                <q-input
                  ...
                  stack-label
                  v-model="
                    ProgramacionOrden.Duracioncalibracion_programacionorden
                  "
                  type="number"
                ...
                />
                <q-select
                  class="col-xs-12 col-md-12 q-field--with-bottom"
                  outlined
                  v-model="ProgramacionOrden.Id_parcal"
                  map-options
                  emit-value
                  option-value="Id_parcal"
                  ...
                  <template v-slot:option="scope">
                    <q-item v-bind="scope.itemProps" v-on="scope.itemEvents">
                      <q-item-section>
                        ...
                        <q-item-label caption
                          >Número de observaciones por medidor en cada prueba:
                          {{ scope.opt.Observaciones_parcal }}</q-item-label
                        >
                        <q-item-label caption
                          >Coeficiente de expansión térmico-volumétrico del
                          agua: {{ scope.opt.Coefexpter_parcal }}</q-item-label
                        >
                        <q-item-label caption
                          >Incertidumbre de Coeficiente de expansión
                          térmico-volumétrico del material RVM:
                          {{ scope.opt.Incertcoefexpter_parcal }}</q-item-label
                        >
                        <q-item-label caption
                          >Incertidumbre de comprensibilidad del Agua (KPa):
                          {{ scope.opt.Incertcomprenagua_parcal }}</q-item-label
                        >
                        <q-item-label caption
                          >Coeficiente de expansión térmico-volumétrico del
                          material RVM:
                          {{ scope.opt.Coefexptermat_parcal }}</q-item-label
                        >
                        <q-item-label caption
                          >Compresibilidad del agua (kPa-1):
                          {{ scope.opt.Comprenagua_parcal }}</q-item-label
                        >
                        <q-item-label caption
                          >Incertidumbre de coeficiente de expanción
                          térmico-volumétrico del agua (°C):
                          {{ scope.opt.Incertcoefexptva_parcal }}</q-item-label
                        >
                      </q-item-section>
                    </q-item>
                  </template>
            ...
            <q-input
              ...
              stack-label
              label="Observaciones"
              v-model="ProgramacionOrden.Observacion_programacionorden"
              type="textarea"
              outlined
            />
        ...
        <q-card-section>
          <div class="row q-col-gutter-sm">
              ...
              <template v-slot:option="scope">
                <q-item v-bind="scope.itemProps" v-on="scope.itemEvents">
                  <q-item-section>
                    ...
                    <q-item-label caption
                      >Código Marca:
                      {{ scope.opt.Id_marcamedidor }}</q-item-label
                    >
                  </q-item-section>
                </q-item>
              </template>
            ...
            >
              ...
              <template v-slot:option="scope">
                <q-item v-bind="scope.itemProps" v-on="scope.itemEvents">
                  <q-item-section>
                    <q-item-label
                      v-html="scope.opt.Descripcion_modelomedidor"
                    />
                    <q-item-label caption
                      >Tipo Indicador:
                      {{ scope.opt.Nombre_tipoindicador }}</q-item-label
                    >
                    <q-item-label caption
                      >Principio Físico:
                      {{ scope.opt.Nombre_princfisico }}</q-item-label
                    >
                    <q-item-label caption
                      >Posición Meidor:
                      {{ scope.opt.Nombre_posicionmed }}</q-item-label
                    >
                    <q-item-label caption
                      >Rango M3: {{ scope.opt.Nombre_rangom3 }}</q-item-label
                    >
                    <q-item-label caption
                      >Diametro Nominal:
                      {{
                        scope.opt.Diametronominal_modelomedidor
                      }}</q-item-label
                    >
                    <q-item-label caption
                      >División Escala:
                      {{ scope.opt.Divisionescala_modelomedidor }}</q-item-label
                    >
                  </q-item-section>
                </q-item>
              </template>
            ...
            <q-select
              ...
              map-options
              emit-value
              option-value="Aptocalibrar_ordenentradad"
            ...
            <q-input
              class="col-xs-12 col-sm-6 col-md-4 q-pt-sm"
              stack-label
            ...
            <q-input
              ...
              v-model="ordenEntradaD.Serialmedidor_ordenentradad"
              outlined
            />
            <q-input
              class="col-xs-12 col-sm-6 col-md-4 q-pt-sm"
    ...
    <q-dialog v-model="isDialogComponenteInstrumentos" style="max-width: 650px">
      <q-card>
        <q-bar>
          <div>
            Instrumentos Caudal {{ caudal.Id_caudal }} ({{
              ...
              <template v-slot:body-cell-Modificar_instrumento="props">
                <q-td key="Modificar_instrumento" :props="props" auto-width>
                  <q-btn
                    ...
                    align="center"
                    outline
                    @click="cambiarInstrumento(props.row)"
                    v-if="props.row"
                  >
                  </q-btn>
                </q-td>
    ...
    <q-dialog
      v-model="isDialogComponenteCambiarInstrumento"
              ...
              <template v-slot:body-cell-Modificar_instrumento="props">
                <q-td key="Modificar_instrumento" :props="props" auto-width>
                  <q-btn
                    ...
                    align="center"
                    outline
                    @click="modificarInstrumento(props.row)"
                    v-if="props.row"
                  >
                  </q-btn>
                </q-td>
    ...
    <q-dialog v-model="isDialogComponenteConsulta" style="max-width: 650px">
      <q-card>
        ...
        <q-card-section>
          ...
          />
          <q-toggle
            class="col-xs-12 col-sm-4 col-md-3 q-field--with-bottom q-pt-sm"
            v-model="todasFechas"
            checked-icon="check"
            color="green"
            :true-value="1"
          ...
          <div class="row justify-center q-pa-sm">
            <q-btn
              ...
              align="center"
              unelevated
              @click="consultarordenProgramacionFechas()"
            >
            </q-btn>
            <q-btn
              label="Regresar"
          ...
          <div class="col-xs-12 col-sm-4 col-md-4 q-field--with-bottom">
            <q-table
              class="col-md-12"
              dense
              title="Resumen Ordenes de Entrada"
              no-data-label="No hay registros"
              hide-bottom
              :rows-per-page-options="[0]"
              flat
            ...
            >
              <template v-slot:bottom-row>
                <q-tr>
                  <q-td colspan="2" align="center"> </q-td>
                  <q-td align="left">
                    <b> Total Registros </b>
                  </q-td>
                  <q-td align="left">
          ...
          <div class="q-pa-md q-gutter-md">
            <q-list>
              <q-item
                v-for="p in listaProgramaciones"
                :key="p.Id_programacionorden"
                @click="programacionOrdenImprimirPDF(p.Id_programacionorden)"
                class="q-my-sm"
                clickable
              >
                ...
                <q-item-section>
                  ...
                  <q-item-label>
                    ...
                    FechaCalibracion:
                    {{ p.Fechacalibracion_programacionorden }} <br />
                    HoraCalibracion: {{ p.Horacalibracion_programacionorden }}
                    <br />
...
export default {
  ...
  data() {
    return {
      ...
      columnsInstrumentos: [
        {
          name: "Modificar_instrumento",
          label: "Modificar",
          field: "Modificar_instrumento",
          sortable: true,
        },
        {
          name: "Id_instrumento",
      ...
      ProgramacionOrdenB: {
        ...
        Horacalibracion_programacionorden: "",
        Duracioncalibracion_programacionorden: 0,
        Medidores_programacionorden: -1,
        Supervisor_programacionorden: "",
        Encargado_programacionorden: "",
        Observacion_programacionorden: "",
        Login_programacionorden: "",
        Trazabilidad_programacionorden: -1,
        FechaIni: "",
        FechaFin: "",
        LisProgOrdenDet: [],
        LisInstProg: [],
  ...
  methods: {
    ...
    mostrarMarcaMedidor(id_marcamedidor, nombre_marcamedidor) {
      ...
      api
        .get(
          `/medidor/marcaMedidorMostrar/${id_marcamedidor}/${nombre_marcamedidor}`
        )
        .then((response) => {
    ...
    mostrarNormaReferencia(norma) {
      ...
      self.$q.loading.show();
      api
        .post("/medidor/normaReferenciaMostrar/", norma)
        .then((response) => {
    ...
    mostrarBancos(banco) {
      ...
      self.$q.loading.show();
      api
        .post("/instrumento/bancosMostrarFiltro/", banco)
        .then((response) => {
    ...
    mostrarParametrosCalibracion(parcal) {
      ...
      self.$q.loading.show();
      api
        .post("/programacionorden/parametrosCalibracionMostrarFiltro/", parcal)
        .then((response) => {
    ...
    mostrarTipoEnsayo(tipoEns) {
      ...
      self.$q.loading.show();
      api
        .post("/instrumento/tipoEnsayoMostrar/", tipoEns)
        .then((response) => {
    ...
    getSelectedString() {
      return this.selectedMedidores.length > this.banco.Medidores_banco
        ...
        ? ""
        : `${this.selectedMedidores.length} medidor${
            this.selectedMedidores.length > 1 ? "es" : ""
    ...
    consultarFiltro() {
      const self = this;
      self.$q.loading.show();
      self.ordenEntradaD.Clasemetrologica_ordenentradad =
        self.condicionMontada.Id_rangomedicion;
      self.ordenEntradaD.Diametronominal_ordenentradad =
        self.condicionMontada.Diametro_medidor;
      self.ordenEntradaD.CodigoSuscriptor = -1;
      ...
      api
        .post("/ordenentrada/OrdenentradadetMostrarFiltro/", self.ordenEntradaD)
        .then((response) => {
