        ...
        <q-form ref="formEntregaMedidores">
          <div class="q-pa-md">
            <q-stepper
              ...
              <q-step
                  :name="1"
                  title="1. Seleccione Informes para imprimir"
                  icon="search"
                  :done="step > 1"
              >
                  ...
                  <q-table class="col-xs-12 col-sm-12 col-md-12" title="" no-data-label="No hay registros" show-bottom flat bordered
                    ...
                    <template v-slot:body-cell-ImprimirCertificado="props">
                      <q-td key="ImprimirCertificado" :props="props" auto-width>
                        <q-btn icon="print" color="black" align="center" outline @click="imprimirInformePDF(props.row.IdInforme)"/>
                      </q-td>
                    </template>
                    <template v-slot:top="props">
                      <q-btn
  ...
  export default {
    ...
    data () {
      return {
        ...
        listaClientes: [],
        columnsCertificado: [
          { name: 'IdInforme', label: 'Id', field: 'IdInforme', sortable: true },
          { name: 'IdOrdenEntradad', align: 'left', label: 'IdOrdenEntradad', field: 'IdOrdenEntradad', sortable: true },
          { name: 'NInforme', label: 'NInforme', field: 'NInforme', sortable: true },
          { name: 'SerialMedidor', label: 'Serial Medidor', field: 'SerialMedidor', sortable: true },
        ...
        vcCertificado: ['IdInforme', 'NInforme', 'SerialMedidor']     
      }
    },
    mounted () {
    ...
    methods: {
      ...
      entregarMedidores () {
        const self = this
        console.log(self.SelectedCertificados)
          let Informes = []
          for (const em of self.SelectedCertificados) {
            Informes.push(em.IdInforme)
          }
          if (Informes.length > 0) {
            self.imprimirCertificadoMasivoPDF(Informes)
          }
          this.step = 2
          return
      ...
      guardarEntrega () {
        ...
        this.$q.dialog({
          ...
          .then((response) => {
            self.imprimirOrdenEntregaPDF(response.data.IdEntregador)
            self.isDialogComponenteGuardarEntrega = falseMedi
            let certificados = []
            for (const em of self.SelectedCertificados) {
      ...
      imprimirInformePDF (idInforme) {
        this.$q.loading.show()
        api.get(`/medidor/ImprimirInforme/${idInforme}`, { responseType: 'arraybuffer' })
          .then(response => {
      ...
      imprimirCertificadoMasivoPDF (certificados) {
        this.$q.loading.show()
        api.post('/medidor/InformesImprimirMasivoPDF/', certificados, { responseType: 'arraybuffer' })
          .then(response => {
            const blob = new Blob([response.data], { type: 'application/pdf' })
            const blobURL = URL.createObjectURL(blob)
            window.open(blobURL)
     ...
     formatoNumero (numero, decimales) {
      ...
      consultarFiltroEntregar (consultaGeneral, isEntregado, todasFechas, fechaIni, fechaFin) {
        ...
        api.get(`/medidor/mostrasInformeMedidores/${todasLasFechas}/${fechaIni}/${fechaFin}/${Login}`)
          .then((response) => {
